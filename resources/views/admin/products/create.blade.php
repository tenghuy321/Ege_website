<x-app-layout>
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Create Our Product</h2>
        <form action="{{ route('ourproduct.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input value="{{ old('product_name') }}" type="text" name="product_name" id="product_name"
                    class="mt-1 block w-full p-2 border rounded-md focus:ring-green-500 focus:border-green-500 text-green-900 text-sm" placeholder="Your Product Name">
                        ...
                        <div class="flex flex-col mb-4 key_specifications-item">
                            ...
                            <div class="mb-2">
                                <input type="text" name="key_specifications[{{ $index }}][title]"
                                    ...
                                    placeholder="Key Specification Title" value="{{ $item->title ?? '' }}">
                            ...
                            <div class="mb-2">
                                <textarea name="key_specifications[{{ $index }}][body]"
                                    ...
                                    placeholder="Key Specification Body">{{ $item->body ?? '' }}</textarea>
                                <p class="mt-2 text-sm text-green-600 dark:text-green-500">
                        ...
                        <div class="flex flex-col mb-4 benefits-item">
                            ...
                            <div class="mb-2">
                                <input type="text" name="benefits[{{ $index }}][title]"
                                    class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 mb-2"
                                    placeholder="Benefits Title" value="{{ $item->title ?? '' }}">
                                <p class="mt-2 text-sm text-green-600 dark:text-green-500">
                            ...
                            <div class="mb-2">
                                <textarea name="benefits[{{ $index }}][body]"
                                    class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                                    placeholder="Benefits Body">{{ $item->body ?? '' }}</textarea>
                                <p class="mt-2 text-sm text-green-600 dark:text-green-500">
                        ...
                        <div class="flex flex-col mb-4 use_cases-item">
                            ...
                            <div class="mb-2">
                                <input type="text" name="use_cases[{{ $index }}][title]"
                                    class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 mb-2"
                                    placeholder="Use Case Title" value="{{ $item->title ?? '' }}">
                                <p class="mt-2 text-sm text-green-600 dark:text-green-500">
                            ...
                            <div class="mb-2">
                                <textarea name="use_cases[{{ $index }}][body]"
                                    class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                                    placeholder="Use Case Body">{{ $item->body ?? '' }}</textarea>
                                <p class="mt-2 text-sm text-green-600 dark:text-green-500">
    ...
    <script>
        ...
        let KeySpecificationItemIndex = {{ count(old('key_specifications', json_decode($product->key_specifications ?? '[]'))) ?? 0 }};
        let BenefitsItemIndex = {{ count(old('benefits', json_decode($product->benefits ?? '[]'))) ?? 0 }};
        let UseCaseItemIndex = {{ count(old('use_cases', json_decode($product->use_cases ?? '[]'))) ?? 0 }};
            ...
            newItem.innerHTML = `
                <div class="mb-2">
                    <input type="text" name="key_specifications[${KeySpecificationItemIndex}][title]"
                           class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 mb-2"
                           placeholder="KeySpecification Title"
                           required>
                </div>
                <div class="mb-2">
                    <textarea name="key_specifications[${KeySpecificationItemIndex}][body]"
                              class="text-green-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5"
                              placeholder="KeySpecification Body"
                              required></textarea>
                </div>
