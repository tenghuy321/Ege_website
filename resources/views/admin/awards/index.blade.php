            ...
            <table class="min-w-full border border-gray-200">
                <thead class="bg-[#415464] text-white">
                    <tr>
                        ...
                        <th class="text-left py-3 px-4 text-[12px]">Title</th>
                ...
                <tbody x-ref="tableBody" class="text-gray-700">
                    @foreach ($awards as $index => $award)
                        <tr draggable="true" x-bind:data-id="{{ $award->id }}"
                            @dragstart="dragStart($event, {{ $award->id }})" @dragover.prevent
                            @drop="drop($event, {{ $award->id }})" class="cursor-move">

                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px]">{{ $index + 1 }}</td>
                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px]">{{ $award->title }}</td>
                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px]">
                                <img src="{{ asset($award->image) }}" class="w-20 h-20 object-contain">
                            </td>
                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px] font-[700]">{{ $award->order }}</td>
                            <td class="text-left py-3 px-4 text-[10px] md:text-[12px] truncate max-w-[200px]">
                                <div class="flex items-center">
                                    <a href="{{ route('award.delete', $award->id) }}" title="Delete"
                                        onclick="event.preventDefault(); deleteRecord('{{ route('award.delete', $award->id) }}')">
                                    ...
                                    <a href="{{ route('award.edit', $award->id) }}" title="Edit">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('award.reorder') }}", {
                                    method: "POST",
                                    headers: {
