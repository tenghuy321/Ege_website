<x-app-layout>
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Award</h2>
        <form action="{{ route('award.update', $award->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input value="{{ old('title', $award->title) }}" type="text" name="title" id="title"
            ...
            <!-- Dropzone for Image -->
            <div class="">
                <label for="dropzone-file{{ $award->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url({{ asset($award->image) }})">
                    ...
                    <input id="dropzone-file{{ $award->id }}" type="file" class="hidden" name="image" accept="image/*"
